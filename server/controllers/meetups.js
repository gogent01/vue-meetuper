...
exports.joinMeetup = function (req, res) {
  ...
  Meetup.findById(id, (errors, meetup) => {
    ...
    return Promise.all([meetup.save(), User.updateOne({_id: user.id}, {$push: {joinedMeetups: meetup}})])
...
exports.leaveMeetup = function (req, res) {
  ...
  Promise.all([Meetup.updateOne({_id: id}, {$pull: {joinedPeople: user.id}, $inc: {joinedPeopleCount: -1}}),
